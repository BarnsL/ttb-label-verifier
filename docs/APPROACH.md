...
- **Government Warning.** Canonical text is taken verbatim from 27 CFR §16.21. The body is compared word-for-word (case-insensitive on wording), while `GOVERNMENT WARNING:` is required in capitals (§16.22). The model is instructed to transcribe the warning exactly as printed so the deterministic check is meaningful. Font-size/legibility-of-type rules beyond capitalization are out of scope for v1.
...
- **Latency depends on the model.** Opus is the accurate default; if it exceeds 5s in practice, `ANTHROPIC_MODEL=claude-haiku-4-5` is a one-line switch.
